...
#            print("c:",c)
            ...
            try:
                ...
                distance_cone=cal.calculate_radius([odo_msg.pose.pose.position.x,odo_msg.pose.pose.position.y],[c.position.x_val,c.position.y_val])
...
#                print("distance_cone",distance_cone)
        ...
        try:   
            ...
            dis_between_blue_cone=cal.calculate_radius(blue_cone_close_1,blue_cone_close_2)
            v_cone_b=np.array(blue_cone_close_1)-np.array(blue_cone_close_2)
            r_cone_b=cal.calculate_radius(v_cone_b,[0,0])
            
        except:
            
            print(blue_cone_close_1,blue_cone_close_2)
            
        sin_projection_blue=cal.calculate_projection(0,dis_close_blue_cone_1,dis_close_blue_cone_2,dis_between_blue_cone)[1]
...
#            print("c:",c)
        ...
        try:   
            ...
            dis_between_yellow_cone=cal.calculate_radius(yellow_cone_close_1,yellow_cone_close_2)
            v_cone_y=np.array(yellow_cone_close_1)-np.array(yellow_cone_close_2)
            r_cone_y=cal.calculate_radius(v_cone_y,[0,0])
        ...
        sin_projection_yellow=cal.calculate_projection(0,dis_close_yellow_cone_1,dis_close_yellow_cone_2,dis_between_yellow_cone)[1]
...
#        print("speed_projection:",speed_projection)
        ...
        try:
            ...
            distance_coneback=cal.calculate_radius([odo_msg.pose.pose.position.x,odo_msg.pose.pose.position.y],[coneback[0].position.x_val,coneback[0].position.y_val])       
...
#        print("sin_projection_yellow",sin_projection_yellow)
        ...
        noise_factor_blue = 1.1/(1.+np.exp(5*(sin_projection_blue-1.)))-0.1 
        noise_factor_yellow = 1.1/(1.+np.exp(5*(sin_projection_yellow-1.)))-0.1 
...
#            action[1]=action[1]-noise_factor_blue
            action[1]=-noise_factor_blue
            
        if sin_projection_yellow<noise_bound:
            
#            action[1]=action[1]+noise_factor_yellow
            action[1]=noise_factor_yellow
...
#        actor.angle.append(action[2])
        ...
        if  car_state.speed<=velocity_max*1/4:
            
            action[0]=1-np.abs(car_controls.steering)-0.5
...
#        car_controls.throttle=float(actionorigin[0])+(ACTION_BOUND0[1]-ACTION_BOUND0[0])/2
        ...
        car_controls.brake=float(action[0])
...
#            actor.brake.append(action[1])
        ...
        for i in range (len(list_cone_sensored)):
            ...
            cone_sort_temp.append([cal.calculate_sita_of_radius(list_cone_sensored[i],[x_auto,y_auto]),list_cone_sensored[i]])
...
#        print("cone_sort_temp:",cone_sort_temp)
        ...
        for i in range (len(cone_sort_temp)):
            ...
            cone_sort_end.append(cone_vector[i])
...
#        state[5]=np.vstack([eul_msg.x,eul_msg.y,eul_msg.z]).ravel()/math.pi
        ...
        speed_data_agument=[vel_msg.x,vel_msg.y,vel_msg.z]*10
        state[1]=np.vstack(speed_data_agument).ravel()
...
#        print("observation:",observation)
        ...
        reward[0]=5*np.exp(reward_projection)
#        if reward>400:
...
#                                         np.array([reward[1]])[np.newaxis, :],np.array([reward[2]])[np.newaxis, :])
            ...
            if collision==True :
#                
                reward[0]=-pow(np.exp(car_state.speed)*5,1)
...
#        critic.one_ep_step=0
        ...
        print("Imitation learning rate actor:",actor.lr_i)
        ep_total=ep_total+1
        print("total train:",ep_total)
        print("\n")
        summary=False  
        
        np.savetxt("memory.csv", M.data, delimiter=",")
